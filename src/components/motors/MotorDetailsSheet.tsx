...
import { Calculator, Ship, Gauge, Fuel, MapPin, Wrench, AlertTriangle, CheckCircle, FileText, ExternalLink, Download, Loader2, Calendar, Shield, BarChart3, X, Settings, Video, CreditCard, Users, Star, ShoppingCart } from "lucide-react";
...
export default function MotorDetailsSheet({
  ...
  const monthlyPayment = useMotorMonthlyPayment({ 
    motorPrice: typeof price === "number" ? price : 0, 
  ...
  const currentReview = smartReview; // Use for mobile engagement
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute inset-0 flex items-end sm:items-center justify-center sm:p-4">
        <div className="relative bg-white dark:bg-slate-900 w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-2xl sm:rounded-xl md:max-w-3xl lg:max-w-4xl flex flex-col animate-in slide-in-from-bottom-4 sm:zoom-in-95 duration-300">
          ...
          <div className="sticky bottom-0 border-t border-slate-200 dark:border-slate-700 bg-gradient-to-r from-white to-slate-50 dark:from-slate-900 dark:to-slate-800 p-3 sm:p-4 sm:rounded-b-xl">
            {/* Mobile: Enhanced Engagement Layout */}
            <div className="sm:hidden">
              {/* Hero Monthly Payment */}
              {typeof price === "number" && monthlyPayment && (
                <div className="text-center mb-3 p-3 bg-primary/5 dark:bg-primary/10 rounded-lg border border-primary/20">
                  <div className="flex items-center justify-center gap-2 mb-1">
                    <CreditCard className="w-4 h-4 text-primary" />
                    <span className="text-xs uppercase tracking-wide text-primary font-semibold">Monthly Payment</span>
                  </div>
                  <div>
                    <span className="text-2xl font-bold text-slate-900 dark:text-white">{getFinancingDisplay(price * 1.13, activePromo?.rate || null).split('•')[0]}</span>
                    {activePromo && activePromo.rate < 7.99 && (
                      <div className="text-xs text-primary font-medium mt-1">
                        ⚡ Special {activePromo.rate}% APR Rate
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-xs text-green-600 font-medium">
                      {Math.floor(Math.random() * 15) + 8} customers viewed this week
                    </span>
                  </div>
                </div>
                <div className="text-right">
                  {promoText && (
                    <div className="bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-400 px-2 py-1 rounded-full text-xs font-semibold mb-1">
                      🔥 {promoText}
                    </div>
                  )}
                  <div className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 px-2 py-1 rounded-full text-xs font-medium">
              ...
              {currentReview && (
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-2 mb-3 border border-slate-200 dark:border-slate-700">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-600 dark:text-slate-400 font-medium">Recent Customer</span>
                  </div>
                  <p className="text-xs text-slate-700 dark:text-slate-300 leading-relaxed line-clamp-2">
                    "{currentReview.comment}"
                  </p>
                  <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">
                    — {currentReview.reviewer}, {currentReview.location}
                  </p>
                </div>
              )}
              ...
              <div className="flex gap-2">
                <button onClick={handleCalculatePayment} className="flex-1 py-3 px-3 text-sm bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700 transition-all duration-200 font-medium flex items-center justify-center gap-2">
                  <Calculator className="w-4 h-4" />
                  Calculate Payment
                </button>
                <Button onClick={handleSelectMotor} className="flex-1 py-3 px-3 text-sm font-semibold shadow-lg hover:shadow-xl transition-all duration-200">
