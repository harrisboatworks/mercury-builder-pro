...
export const HPCategorySelector = ({
  ...
  return <div className="space-y-8">
      ...
      <div className="space-y-4">
        {showBackButton && onBack && <button onClick={onBack} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Select the horsepower range that matches your boating needs</p>
        </div>
      </div>

      {/* Category Grid - Porsche-inspired layout */}
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 max-w-7xl mx-auto">
        {HP_CATEGORIES.map(category => <Card key={category.id} className="group cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 border-0 bg-gradient-to-br from-background to-muted/30 overflow-hidden" onClick={() => onCategorySelect(category)}>
            <CardContent className="p-0">
              ...
              <div className="relative h-48 bg-gradient-to-br from-primary/10 to-primary/5 flex items-center justify-center overflow-hidden">
                ...
                <img src={category.heroImage} alt={`${category.name} boat type`} className="w-24 h-24 object-contain opacity-80 group-hover:opacity-100 transition-all duration-300 group-hover:scale-110" />
