...
export default function KoreanBlogArticlePage() {
  const { slug } = useParams<{ slug: string }>();
  const article = slug ? getKoreanArticleBySlug(slug) : undefined;
      ...
      <main className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6">
          <span className="flex items-center gap-1">
            ...
            {new Date(article.datePublished).toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' })}
          </span>
          <span className="flex items-center gap-1">
        ...
        {article.faqs && article.faqs.length > 0 && (
          <section className="mt-12 mb-12">
            <h2 className="text-2xl font-light text-foreground mb-6">자주 묻는 질문</h2>
        ...
        <section className="text-center bg-primary/5 rounded-2xl p-8 mb-12">
          <h2 className="text-xl font-light text-foreground mb-3">지금 바로 견적을 확인하세요</h2>
          <p className="text-muted-foreground text-sm mb-6">5분이면 온라인으로 견적을 받을 수 있습니다</p>
