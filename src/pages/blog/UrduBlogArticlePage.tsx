...
export default function UrduBlogArticlePage() {
  const { slug } = useParams<{ slug: string }>();
  const article = slug ? getUrduArticleBySlug(slug) : undefined;
  ...
  return (
    <div className="min-h-screen bg-background" lang="ur" dir="rtl">
      <Helmet>
      ...
      <BlogHreflangLinks locale="ur" slug={article.slug} />
      <LuxuryHeader />
      ...
      <main className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
        ...
        <LanguageSwitcher currentLang="ur" currentSlug={article.slug} />

        <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6">
          <span className="flex items-center gap-1">
            <Calendar className="w-4 h-4" />
            {new Date(article.datePublished).toLocaleDateString('ur-PK', { year: 'numeric', month: 'long', day: 'numeric' })}
          </span>
          <span className="flex items-center gap-1">
        ...
        <div className="mb-8 pb-4 border-b border-border">
          <AuthorByline name="Jay Harris" title="Harris Boat Works کے مالک" byLabel="مصنف" bioLabel="مصنف کا تعارف" />
        </div>
        ...
        {article.faqs && article.faqs.length > 0 && (
          <section className="mt-12 mb-12">
            <h2 className="text-2xl font-light text-foreground mb-6">اکثر پوچھے گئے سوالات</h2>
        ...
        <section className="text-center bg-primary/5 rounded-2xl p-8 mb-12">
          <h2 className="text-xl font-light text-foreground mb-3">ابھی اپنا کوٹ حاصل کریں</h2>
          <p className="text-muted-foreground text-sm mb-6">5 منٹ میں آن لائن کوٹ حاصل کریں</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              to="/quote/motor-selection"
              className="inline-flex items-center justify-center px-6 py-3 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity"
            >
              آن لائن کوٹ حاصل کریں
            </Link>
            <a
