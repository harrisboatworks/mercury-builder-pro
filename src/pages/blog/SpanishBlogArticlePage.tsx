...
export default function SpanishBlogArticlePage() {
  const { slug } = useParams<{ slug: string }>();
  const article = slug ? getSpanishArticleBySlug(slug) : undefined;
      ...
      <main className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
        ...
        <div className="flex items-center gap-4 text-sm text-muted-foreground mb-6">
          <span className="flex items-center gap-1">
            ...
            {new Date(article.datePublished).toLocaleDateString('es-419', { year: 'numeric', month: 'long', day: 'numeric' })}
          </span>
          <span className="flex items-center gap-1">
        ...
        {article.faqs && article.faqs.length > 0 && (
          <section className="mt-12 mb-12">
            <h2 className="text-2xl font-light text-foreground mb-6">Preguntas frecuentes</h2>
        ...
        <section className="text-center bg-primary/5 rounded-2xl p-8 mb-12">
          <h2 className="text-xl font-light text-foreground mb-3">Obtenga su cotización en línea</h2>
          <p className="text-muted-foreground text-sm mb-6">Configure su motor Mercury y vea el precio real en minutos</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              to="/quote/motor-selection"
              className="inline-flex items-center justify-center px-6 py-3 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 transition-opacity"
            >
              Ver precios en línea
            </Link>
            <a
