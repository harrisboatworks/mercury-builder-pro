...
describe('deposit deal-packet migration', () => {
  ...
  it('creates a private delivery table with admin read and no public write policy', () => {
    ...
    expect(migration).toContain('It is not proof that every required email was sent.');
    ...
    expect(migration).toContain('REVOKE ALL ON FUNCTION public.reconcile_deposit_notification_status(uuid) FROM PUBLIC, anon, authenticated, service_role');
    expect(migration).toContain('GRANT EXECUTE ON FUNCTION public.reconcile_deposit_notification_status(uuid) TO service_role');
    expect(migration).toContain("IF CURRENT_USER IS DISTINCT FROM 'service_role' THEN");
    ...
    expect(migration).not.toMatch(/ON public\.deposit_email_deliveries\s+FOR UPDATE/);
