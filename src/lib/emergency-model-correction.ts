...
export async function emergencyModelCorrection(): Promise<CorrectionResult> {
  ...
  try {
    console.log('🚨 EMERGENCY MODEL CORRECTION STARTING...');
    ...
    console.log(`📊 Found ${brochureMotors.length} brochure motors to process`);

    // 2. Group motors by HP and family to identify duplicates
    const motorGroups = new Map<string, any[]>();
    
    for (const motor of brochureMotors) {
      const key = `${motor.horsepower}-${motor.family || 'FourStroke'}`;
    ...
    for (const [groupKey, motors] of motorGroups) {
      console.log(`\n🔍 Processing group: ${groupKey} (${motors.length} motors)`);
      
      if (motors.length === 1) {
        // Single motor - validate and correct if needed
        const motor = motors[0];
        await processSingleMotor(motor, result);
      } else {
    ...
    console.log('\n✅ EMERGENCY CORRECTION COMPLETE');
    console.log(`📈 Summary: ${result.motorsCorrected} corrected, ${result.motorsDeleted} deleted`);
...
async function processSingleMotor(motor: any, result: CorrectionResult) {
  ...
  if (!validation.isValid && validation.correctedModelNumber) {
    console.log(`🔧 Correcting ${motor.id}: ${motor.model_number} → ${validation.correctedModelNumber}`);
...
async function processDuplicateMotors(motors: any[], result: CorrectionResult) {
  console.log(`🔥 Found ${motors.length} duplicate motors`);
  ...
  for (const motor of motorsToDelete) {
    console.log(`🗑️ Deleting duplicate: ${motor.id} (${motor.model_number})`);
...
export async function addModelNumberProtection(): Promise<void> {
  console.log('🔒 Adding model number protection...');
  
  // This would be implemented as a database function/trigger
  // For now, we'll add validation in the application layer
  console.log('⚠️ Model number protection should be implemented at database level');
}
