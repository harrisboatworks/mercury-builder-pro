...
export async function emergencyModelCorrection(): Promise<CorrectionResult> {
  ...
  try {
    ...
    console.log('📋 Using official Mercury reference with 167 models');
    ...
    for (const motor of brochureMotors) {
      ...
      if (isAutoGenerated || hasSuffix || isInvalidOfficial) {
        ...
        console.log(`🗑️ Marking for deletion: ${motor.model_number} (${motor.model_display}) - ${isAutoGenerated ? 'auto-generated' : hasSuffix ? 'has suffix' : 'invalid official'}`);
      } else {
    ...
    console.log(`\n🧹 Deleting ${motorsToDelete.length} invalid motors...`);
