...
interface MotorComparisonContextValue {
  comparisonList: ComparisonMotor[];
  addToComparison: (motor: Motor | ComparisonMotor) => void;
  removeFromComparison: (motorId: string) => void;
  clearComparison: () => void;
  isInComparison: (motorId: string) => boolean;
  toggleComparison: (motor: Motor | ComparisonMotor) => void;
  count: number;
  isFull: boolean;
  maxMotors: number;
  getShareableUrl: () => string;
  shareComparison: () => Promise<void>;
  saveComparison: (name?: string) => Promise<SavedComparison | null>;
  savedComparisons: SavedComparison[];
  loadSavedComparisons: () => Promise<void>;
  deleteSavedComparison: (comparisonId: string) => Promise<void>;
  loadingSaved: boolean;
}

const MotorComparisonContext = createContext<MotorComparisonContextValue | null>(null);

export function MotorComparisonProvider({ children }: { children: ReactNode }) {
  ...
  }, [comparisonList, loadSavedComparisons]);
  ...
  const value: MotorComparisonContextValue = {
    comparisonList,
    addToComparison,
  ...
  return (
    <MotorComparisonContext.Provider value={value}>
      {children}
    </MotorComparisonContext.Provider>
  );
}

export function useMotorComparison() {
  const context = useContext(MotorComparisonContext);
  if (!context) {
    throw new Error('useMotorComparison must be used within a MotorComparisonProvider');
  }
  return context;
}
