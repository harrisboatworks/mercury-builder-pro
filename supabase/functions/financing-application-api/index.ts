  ...
  try {
    ...
    if (input.action === 'save') {
      ...
      if (applicationId && resumeToken) {
        ...
        // Claim an anonymous draft only if it is still unowned. A concurrent
        // authenticated resume must never replace the owner selected first.
        if (existingDraft.user_id === null && userId !== null) {
    ...
    if (input.applicationId && input.resumeToken) {
      ...
      if (existingState.user_id === null && userId !== null) {
