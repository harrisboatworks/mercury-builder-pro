 ...
 *      a dedicated table that only service_role can write, and the caller never
 *      chooses the target row's contents.
 *
...
/**
 * Stable admin key: one logical send per quote + template.
 * A deliberate resend appends `:resend:{nonce}`.
 */
 ...
 * dedupes while a later customer retry can still go out.
 */
