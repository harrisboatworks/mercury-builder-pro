...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const body = {
      ...
      notice:
        'Read-only public feed. Prices in CAD. `sellingPrice` is the agent-visible current CAD retail (resolved via manual overrides > sale_price > dealer_price > msrp); `msrp` is included for comparison. Final out-the-door price requires human confirmation. Pickup only at Gores Landing, ON. Mercury Verado is special-order only and not in the default inventory feed — contact us by phone or email for Verado quotes.',
      motors,
    };
