-- Fix the ON CONFLICT clause in update_brochure_models_bulk to match the actual constraint
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    ON CONFLICT (model_number) WHERE (is_brochure = true AND model_number IS NOT NULL AND model_number != '')
    DO UPDATE SET
