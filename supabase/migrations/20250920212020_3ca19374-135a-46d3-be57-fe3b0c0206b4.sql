-- Safe Mercury inventory automation setup
-- Clean up any existing jobs that might exist (ignore errors if they don't exist)

DO $$
BEGIN
  -- Try to unschedule existing jobs, ignore errors if they don't exist
  BEGIN
    PERFORM cron.unschedule('mercury-inventory-sync');
  EXCEPTION WHEN OTHERS THEN
    -- Job doesn't exist, continue
  END;
  
  BEGIN
    PERFORM cron.unschedule('daily-inventory-sync');
  EXCEPTION WHEN OTHERS THEN
    -- Job doesn't exist, continue  
  END;
  
  BEGIN
    PERFORM cron.unschedule('mercury-daily-inventory-sync');
  EXCEPTION WHEN OTHERS THEN
    -- Job doesn't exist, continue
  END;
END $$;
...
-- Create policy for admin access to cron logs (drop if exists first)
DROP POLICY IF EXISTS "Admins can manage cron logs" ON cron_job_logs;
