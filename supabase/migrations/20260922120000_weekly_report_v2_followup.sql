-- Follow-up to report v2: separate non-quote leads and credit later motor selections.
...
with weeks as (
  ...
  select i, session_id, min(created_at) as first_at,
    ...
    max(created_at) filter(where event_type='motor_selected') as last_motor_at,
  ...
  select * from quote_base where not marked_test and not api_quote and not admin_quote
    and coalesce(lead_source,'') not in ('repower_guide','ai_chat')
), sources as (
  ...
  -- The latest selection is after a visit iff any later selection exists.
  -- Reuse the session aggregate rather than scanning materialized events per article.
  select b.i,b.path,count(distinct b.session_id) as sessions,
    count(distinct b.session_id) filter(where s.last_motor_at>b.blog_at) as quote_starts
 ...
 select w.i,jsonb_build_object(
   ...
   'guide_leads',(select count(*) from quote_base q where q.i=w.i and not marked_test and not admin_quote and lead_source='repower_guide'),
   'chat_leads',(select count(*) from quote_base q where q.i=w.i and not marked_test and not admin_quote and lead_source='ai_chat'),
