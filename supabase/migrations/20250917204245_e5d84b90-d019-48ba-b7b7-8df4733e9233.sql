...
-- Create temporary function to extract and apply Mercury model numbers
    ...
    LOOP
        ...
        BEGIN
            ...
            IF mercury_model_number IS NOT NULL THEN
                ...
                RANGE NOTICE '[MERCURY] Updated motor ID % (%.1f HP) "%s" -> model_number: %', 
                    rec.id, rec.horsepower, clean_display, mercury_model_number;
            ELSE
                RAISE NOTICE '[MERCURY] No match found for motor ID % (%.1f HP): "%s"', 
                    rec.id, rec.horsepower, clean_display;
            END IF;
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '[MERCURY] Error updating motor ID %: %', rec.id, SQLERRM;
        END;
    END LOOP;
