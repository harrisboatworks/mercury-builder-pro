...
--
-- MUST be applied before send-quote-email is deployed with the audit/claim
-- code. This repo has one production database and no staging.
--
...
--
-- Replaces the notes-based send record in send-quote-email. An earlier version
-- of that write was keyed on a caller-supplied quote_number from a public
-- (verify_jwt = false) function using the service-role client. That has since
-- been narrowed on main: the write is now behind requireAdmin and keyed on the
-- full quote UUID (.eq('id', quoteId)), so the overwrite path is already closed.
--
-- What remains wrong with notes-as-audit, and what this table fixes: a single
-- free-text field cannot record a failed send, cannot be deduplicated, is
-- overwritten by the next send, and is written only when an admin happens to be
-- the caller. Customer-initiated sends were recorded nowhere at all.
--
...
--
-- quote_number is a display label only (HBW-###### or a UUID prefix printed in
-- the email). Lookups MUST use quote_id. Two UUIDs can share an 8-hex prefix.
...
-- Admin-readable audit trail. Look up by the full quote UUID, never by the
-- display quote_number (an 8-hex UUID prefix can collide).
CREATE OR REPLACE FUNCTION public.get_quote_email_deliveries_v1(_quote_id uuid)
...
REVOKE ALL ON FUNCTION public.complete_quote_email_delivery_v1(uuid, text, text, text, text)
  FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION public.get_quote_email_deliveries_v1(uuid) FROM PUBLIC, anon;
...
GRANT EXECUTE ON FUNCTION public.complete_quote_email_delivery_v1(uuid, text, text, text, text)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.get_quote_email_deliveries_v1(uuid) TO authenticated, service_role;
