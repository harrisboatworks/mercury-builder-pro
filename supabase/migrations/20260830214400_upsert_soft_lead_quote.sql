...
BEGIN
  -- Origin/Referer is a browser CSRF/CORS boundary, not authentication. The
  -- high-entropy session capability below is independently bound to a header.
  request_origin := pg_catalog.lower(COALESCE(
  ...
  -- Keep this exact owned-origin list in sync with
  -- supabase/functions/_shared/browser-origin.ts. Generated Vercel branch or
  -- deployment aliases and broad hosting-provider wildcards are not trusted.
  IF NOT (
    request_origin = 'https://www.mercuryrepower.ca'
    OR request_origin LIKE 'https://www.mercuryrepower.ca/%'
    OR request_origin = 'https://mercuryrepower.ca'
    OR request_origin LIKE 'https://mercuryrepower.ca/%'
    OR request_origin = 'https://quote.harrisboatworks.ca'
    OR request_origin LIKE 'https://quote.harrisboatworks.ca/%'
    OR request_origin = 'https://www.mercuryquote.ca'
    OR request_origin LIKE 'https://www.mercuryquote.ca/%'
    OR request_origin = 'https://mercuryquote.ca'
    OR request_origin LIKE 'https://mercuryquote.ca/%'
    OR request_origin = 'https://mercury-builder-pro.vercel.app'
    OR request_origin LIKE 'https://mercury-builder-pro.vercel.app/%'
    OR request_origin = 'https://mercury-builder-pro-hbw.vercel.app'
    OR request_origin LIKE 'https://mercury-builder-pro-hbw.vercel.app/%'
    OR request_origin = 'https://mercury-builder-pro-git-main-hbw.vercel.app'
    OR request_origin LIKE 'https://mercury-builder-pro-git-main-hbw.vercel.app/%'
  ...
  IF canonical_id IS NULL THEN
    ...
    END LOOP;
  ELSE
    -- Last database arrival wins across tabs. The browser queues writes within
    -- a tab so an older local snapshot cannot overtake a newer one.
...
REVOKE ALL ON FUNCTION public.upsert_soft_lead_quote(text, jsonb)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.upsert_soft_lead_quote(text, jsonb)
