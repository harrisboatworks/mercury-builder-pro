-- Drop the obsolete function with old signature
DROP FUNCTION IF EXISTS public.update_brochure_models_bulk(text[], jsonb[]);

-- Recreate the correct function with comprehensive field handling
...
BEGIN
  ...
  FOR rec IN SELECT * FROM jsonb_array_elements(p_rows) LOOP
    ...
    SET
      ...
      model = COALESCE(rec->>'model', m.model),
      motor_type = COALESCE(rec->>'motor_type', m.motor_type),
...
-- Sanity check: Test with empty array (should return 0)
SELECT public.update_brochure_models_bulk('[]'::jsonb) as empty_test;