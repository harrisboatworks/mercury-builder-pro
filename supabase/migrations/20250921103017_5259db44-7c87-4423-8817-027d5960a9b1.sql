-- Comprehensive Mercury Motor Catalog Correction Migration (Fixed)
...
-- Step 3: Create helper functions for model key generation
...
BEGIN
  ...
  -- Add special designations using proper array concatenation
  IF description ILIKE '%Command Thrust%' THEN
    key_parts := key_parts || ARRAY['CT'];
  END IF;
  
  IF description ILIKE '%ProKicker%' THEN
    key_parts := key_parts || ARRAY['PK'];
  END IF;
  
  IF description ILIKE '%Tiller%' THEN
    key_parts := key_parts || ARRAY['TILLER'];
  END IF;
  
  IF description ILIKE '%DTS%' THEN
    key_parts := key_parts || ARRAY['DTS'];
  END IF;
  
  -- Add rigging code from description
  rigging_match := regexp_match(description, '(ELPT|ELHPT|EXLPT|EXLHPT|ECXLPT|EPT|ELH|EH|MLH|MXLH|MH|EL|XL|XXL|CXXL|CXL)');
  IF rigging_match IS NOT NULL THEN
    key_parts := key_parts || ARRAY[rigging_match[1]];
  END IF;
...
-- Step 4: Import all official Mercury models with correct data
-- Insert the first batch (30 models) to start with proper model numbers, names, and pricing
INSERT INTO motor_models (
  ...
  -- Critical 60HP models that were causing the issue
  ('1F60413GZ', build_official_model_key('1F60413GZ', '60ELPT FourStroke'), '1F60413GZ', 'Outboard', '60 ELPT FourStroke', 12161, 17373, 60, 'Outboard', 2025, true, 'Mercury', 'FourStroke', now(), now()),
  ('1F6041TJZ', build_official_model_key('1F6041TJZ', '60ELHPT FourStroke Tiller'), '1F6041TJZ', 'Outboard', '60 ELHPT FourStroke Tiller', 12826, 18323, 60, 'Outboard', 2025, true, 'Mercury', 'FourStroke', now(), now()),
  ('1F60453GZ', build_official_model_key('1F60453GZ', '60ELPT Command Thrust FourStroke'), '1F60453GZ', 'Outboard', '60 ELPT Command Thrust FourStroke', 12469, 17813, 60, 'Outboard', 2025, true, 'Mercury', 'FourStroke', now(), now()),
  ('1F60463GZ', build_official_model_key('1F60463GZ', '60EXLPT Command Thrust FourStroke'), '1F60463GZ', 'Outboard', '60 EXLPT Command Thrust FourStroke', 12815, 18307, 60, 'Outboard', 2025, true, 'Mercury', 'FourStroke', now(), now()),
  ('1F6045TJZ', build_official_model_key('1F6045TJZ', '60ELHPT Command Thrust FourStroke Tiller'), '1F6045TJZ', 'Outboard', '60 ELHPT Command Thrust FourStroke Tiller', 13189, 18842, 60, 'Outboard', 2025, true, 'Mercury', 'FourStroke', now(), now())
