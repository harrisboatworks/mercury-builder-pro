...
-- Policies using correct pg_policies columns
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'financing_options' AND policyname = 'Public read access for financing_options'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'financing_options' AND policyname = 'Authenticated users can insert financing_options'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'financing_options' AND policyname = 'Authenticated users can update financing_options'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'financing_options' AND policyname = 'Authenticated users can delete financing_options'
  ) THEN
...
-- Ensure promo image columns exist
