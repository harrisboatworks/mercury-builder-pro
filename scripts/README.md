...
| `validate-schema-org.mjs` | Posts every JSON-LD block in `dist/` to validator.schema.org and fails the build on error-severity issues, network failures, non-JSON responses, or a zero-block run. Schema warnings remain non-blocking. Skippable only via the explicit `SKIP_SCHEMA_ORG_VALIDATOR=1` outage bypass. Sampling caps requests at `SCHEMA_VALIDATOR_MAX_FILES` (default 80) to stay within rate limits. Wired into `npm build` after `check-structured-data.mjs`. |
