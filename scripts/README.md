...
| `deploy-supabase-functions.mjs` | Deploys edge functions changed in a git range (or one slug). Reuses `deployTargetsFromDiff` for `_shared/` fan-out. Continues after per-function failures, then exits 1 if any failed. Never applies migrations. |
