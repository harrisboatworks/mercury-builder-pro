...
The migration test contains PostgreSQL behavior cases plus one manifest assertion. Official PostgreSQL 17 (PGDG `postgresql-17` / `postgresql-client-17`, or Homebrew `postgresql@17`) is required for those cases. `npm run test:cron-auth-pg` sets `CRON_AUTH_REQUIRE_PG=1` and fails closed when `initdb`/`pg_ctl`/`psql` are missing. A skipped run is not migration acceptance evidence. The fixture locates binaries via `pg_config --bindir`, `/usr/lib/postgresql/17/bin`, and Homebrew paths (`scripts/lib/local-postgres17.mjs`), then accepts a directory only when `initdb`, `pg_ctl`, and `psql` all report major 17. Mixed toolchains and other majors are rejected. Fixture evidence records `major` and `version`.

PostgreSQL 17.11 ran against new temporary fake `cron`, `vault`, and `net` schemas through a private Unix socket with TCP disabled. Tests replace only extension setup with fixtures, execute the authored DO block, and never invoke an actual scheduler or HTTP client. They verify exact command-byte/metadata preservation, inactive jobs, unrelated rows, numeric timeout, reruns, rollback after second-job failure, duplicate/missing callers and references, unsupported command shapes, EDGE-over-CRON precedence, valid runtime lookup, and missing/empty/duplicate runtime references failing before fake HTTP. Clusters are stopped and removed afterward.

The fixtures do not establish installed Supabase extension behavior, the live job execution roles' Vault access, secret parity, or production readiness. The committed prerequisite manifest stays `unresolved`.
