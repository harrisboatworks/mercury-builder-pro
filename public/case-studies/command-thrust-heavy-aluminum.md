---
canonical: https://www.mercuryrepower.ca/case-studies/command-thrust-heavy-aluminum
last_updated: 2026-07-14
currency: CAD
pickup_only: true
delivery_offered: false
location: Gores Landing, ON, Canada
final_quote_requires_dealer_confirmation: true
verado_status: special-order only, not in default inventory
case_study_id: CS11
slug: command-thrust-heavy-aluminum
boat_type: "18-foot deep-V aluminum"
region: "Rice Lake / Kawarthas"
---

# Command Thrust 60 HP on Heavy Aluminum

Repower an 18-foot deep-V aluminum with a Mercury 60 HP Command Thrust for $10,000–$13,000 all-in at Harris Boat Works.

## Factbox

- **Boat type:** 18-foot deep-V aluminum
- **Region:** Rice Lake / Kawarthas
- **Scenario:** Heavy-hull Command Thrust repower
- **HP jump:** Same HP, bigger gearcase + high-thrust prop

## Before → After

- **Before:** Standard Mercury 60 HP FourStroke
- **After:** Mercury 60 HP FourStroke Command Thrust

## Recommendation

Best for heavy 17–19-foot deep-V aluminum hulls rated 60 HP that need more low-end pull without exceeding the transom rating or insurance class.

## Why it worked

- Larger gearcase and high-thrust prop move more water
- Hole shot time roughly cut in half on loaded hulls
- Stays inside the boat's 60 HP transom rating

## Customer quote

> Same horsepower on the plate, twice the hole shot under load.

## Quote a similar repower

- HTML page (canonical for humans): https://www.mercuryrepower.ca/case-studies/command-thrust-heavy-aluminum
- Start a Mercury quote: https://www.mercuryrepower.ca/quote/motor-selection

## Notes

- All pricing in CAD. Pickup only at Gores Landing, ON, by the buyer in person with valid government photo ID. No couriers or third parties.
- Final motor recommendation confirmed by Harris Boat Works staff.
- Verado not used in default repower recommendations (special-order only).

