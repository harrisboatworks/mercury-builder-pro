...
- Does not work miracles on light boats. On a 14 ft tinny or a light cruising pontoon, the standard gearcase usually outperforms CT in top end and fuel (see the lake test below).
...
That last point is the most common misconception at the shop. People with an existing standard-gearcase motor often ask if we can "add Command Thrust." The honest answer is rarely yes. For the full retrofit decision, see our [Mercury Command Thrust pontoon guide](/blog/mercury-command-thrust-guide-pontoon-boats).
...
| Boat type | Command Thrust eligible? | Why |
|---|---|---|
...
For the deep pontoon-specific dive, see the [Mercury Command Thrust pontoon guide](/blog/mercury-command-thrust-guide-pontoon-boats).
...
1. **Boat type and loaded weight.** Honest answer about what you actually carry on a typical trip, not the boat's dry weight on the brochure.
2. **Capacity-plate maximum HP.** Adding CT does not let you exceed capacity. We see customers wanting CT on a hull that is already at max HP and that is not the lever they think it is.
3. **Use case (fishing? cruising? water sports? cottage hauling?).** Each shifts the answer.
...
- **I'll just swap the gearcase later if I don't like it.**  
  Retrofitting CT means new gearcase, driveshaft, water pump, prop. The total is usually within striking distance of a new motor. Decide CT vs standard at purchase, not later.
- **More thrust means more horsepower.**  
  No. Same powerhead, same peak HP. CT changes how that power gets delivered to the water, not how much there is.
...
Technically yes, practically rarely worth it. The gearcase, driveshaft, water pump, and prop are all different. By the time you add up parts and labor, you are usually close to the cost of trading toward a new CT motor. We quote both paths at HBW so you can compare.

### Does Command Thrust affect my Mercury warranty?

No. CT is a factory option built on a Mercury outboard. The standard Mercury Marine warranty (3 years limited plus 3 years corrosion concurrent on Canadian-sold motors) applies the same way it would on the standard gearcase model.

### Will Command Thrust burn more fuel?

At cruising RPM, fuel burn is comparable. At trolling speeds, CT can burn slightly more because the bigger prop is moving more water. At hole shot and time-to-plane, CT actually burns less because you get to plane faster and spend less time at the high-fuel-burn pre-plane RPM.

### Is Pro XS Command Thrust different from FourStroke Command Thrust?

Different powerhead, same gearcase concept. Pro XS CT puts the more aggressive Pro XS powerhead on a CT gearcase. Used most often on heavier aluminum or fiberglass rigs that want Pro XS punch under load. FourStroke CT is the broader Mercury family on the same gearcase.

### Do I need a special prop for Command Thrust?

Yes. CT uses a different prop shaft diameter and a different prop family than standard gearcase. Props do not swap between the two. Prop selection is part of the CT install conversation at HBW. We carry common pontoon and aluminum CT props in stock and order specialty pitches as needed.
